 ...
 */
struct dynarray {
  void** data;  //An what is this?
  int size;  //number of the data in the array
...
struct dynarray* dynarray_create() {
  struct dynarray* da = malloc(sizeof(struct dynarray));  //allocate the memory for the WHOLE array structuure 
  da -> data = malloc(2 * sizeof(void*));  //allocate the size of data and since have capacity of 2 
                                           //=> 2 * sizeof(void*)
  da -> size = 0;  //at the beginning, there isn't data in the array so the size is set to 0
  da -> capacity = 2;  //setting the capacity for the array
  return da;  //return the the pointer to the address of the allocated structure
}
...
void dynarray_free(struct dynarray* da) {
    free(da -> data);  //since the data is in the array, need to free tha data in the array frirst
    free(da);  //free the array
  return;
}
...
int dynarray_size(struct dynarray* da) {
  if (da == NULL) {  //check the sife of the array. If the array equal NULL aka 0, return 0
    return 0;
  } else {  
    return da -> size;  //if not NULL, return the size of the array
  }
}
...
void dynarray_insert(struct dynarray* da, void* val) {
  if (da -> size < da -> capacity) {
    ...
    int capacity_new = da -> capacity * 2;
    void** data_new = malloc(capacity_new * sizeof(void*));
    for (int i = 0; i < da -> size; i++) {
      data_new[i] = da -> data[i];
    }
  free(da -> data);
  da -> data = data_new;
  da -> capacity = capacity_new;
  da -> data[da -> size] = val;
  da -> size ++;
  }
  return;
